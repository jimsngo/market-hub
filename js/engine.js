...
export async function fetchNews() {
    try {
        ...
        const macroKeywords = ['FED', 'POWELL', 'INFLATION', 'RATE', 'OIL', 'ENERGY', 'GOLD', 'BITCOIN', 'MARKET'];
...
export async function fetchMarketData(symbols) {
    ...
    for (let sym of symbols) {
        try {
            ...
            if (!raw?.chart?.result?.[0]) throw new Error("Bad Structure");
            ...
            if (ticker === "TNX") results.yield = cur;
            results.indices[ticker] = calc;

        } catch (err) { console.warn(`[SKIP] ${sym}: ${err.message}`); }
